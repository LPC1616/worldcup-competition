...
contract WorldCupV2 is Initializable {
    ...
    event ChangeDeadline(uint256 _oldDeadline, uint256 _newDeadline);
    ...
    function initialize(uint256 _deadline) public initializer {
        admin = msg.sender;
        require(_deadline > block.timestamp, "invalide deadline");
        deadline = _deadline;
    }
    ...
    function changeDeadline(uint256 _newDeadline) external {
        require(_newDeadline > block.timestamp, "invalide deadline");
